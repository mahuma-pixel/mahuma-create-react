import React from "react";

export default function Weather(){return <div className="weather">
    <div className="container">
    <div className="row">
        <form className="form">
        <input type="search" placeholder="Enter a city" className="useForm col-sm-9" />
        <input type="submit" value={'Search'} className="submitForm col-sm-3 btn btn-primary" />
    </form>
</div></div>
<h1>
    San Fransisco
</h1>
<div className="container">
    <div className="row">
        <div className="col-sm-6"> <ul><li>
            Saturday 01:17, smoke</li>
            <li>Humidity: <span className="values">30%</span>, Wind: <span className="values">6.69km/h</span>
                </li></ul></div>
        <div className="col-sm-6"><span><img src="data:image/png;base64," alt="forecast"></img></span>
        <span className="temperature">23</span><span className="unit">°C</span></div>
    </div></div>
    <br/>
    <div className="container">
    <div className="row">
        <div className="col-sm-2">
            <div className="day">Mon</div>
            <div className="forecastImage"><img src="data:image/png;base64," alt="forecast-mon"></img></div>
            <div className="temps"><span className="max">30°</span><span className="min">17°</span></div>
        </div>
        <div className="col-sm-2"><div className="day">Tue</div>
            <div className="forecastImage"><img src="data:image/png;base64," alt="forecast-tue"></img></div>
            <div className="temps"><span className="max">23°</span><span className="min">16°</span></div></div>
        <div className="col-sm-2"><div className="day">Wed</div>
            <div className="forecastImage"><img src="data:image/png;base64," alt="forecast-wed"></img></div>
            <div className="temps"><span className="max">21°</span><span className="min">16°</span></div></div>
        <div className="col-sm-2"><div className="day">Thurs</div>
            <div className="forecastImage"><img src="data:image/png;base64," alt="forecast-thurs"></img></div>
            <div className="temps"><span className="max">18°</span><span className="min">12°</span></div></div>
        <div className="col-sm-2"><div className="day">Fri</div>
            <div className="forecastImage"><img src="data:image/png;base64," alt="forecast-fri"></img></div>
            <div className="temps"><span className="max">23°</span><span className="min">14°</span></div></div>
    </div></div>
</div>}